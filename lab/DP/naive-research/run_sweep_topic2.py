#!/usr/bin/env python3
"""Run the Lab 4 Topic 2 flow-control comparison (CBS vs baselines).

Modes:
  torus3d_dor             algorithm 3, no protection (unsafe baseline)
  torus3d_dor_cbs         algorithm 3 + Critical Bubble Scheme (ours)
  torus3d_adaptive_escape algorithm 4, escape-VC protection (Topic 1)

Unlike the Topic 1 sweep, a gem5 failure (e.g. a deadlock panic) is a
result, not an error: the point is recorded with deadlock=1.
"""
...
def mode_options(mode: str) -> tuple[int, list[str]]:
    if mode == "torus3d_dor":
        return 3, []
    if mode == "torus3d_dor_cbs":
        return 3, ["--enable-cbs"]
    return 4, []
...
def run_point(
    ...
    rate_text = f"{rate:.2f}"
    name = f"{mode}_{pattern}_{rate_text}_v{vcs}_c{sim_cycles}"
    point_run_dir = RUN_DIR / name
    point_run_dir.mkdir(parents=True, exist_ok=True)
    log_path = LOG_DIR / f"{name}.log"
    routing, extra = mode_options(mode)
    command = [
        str(GEM5),
        "-d",
        str(point_run_dir),
