...
We implement the design in gem5 Garnet and evaluate it against an
equal-storage four-private-VC baseline. Across 1,077 main-matrix simulations, all runs
...
measurable: at eight-cycle link latency, a fully skewed workload caps a
private four-VC baseline at a stable offered rate of 0.08, whereas the same
eight buffer slots per pair, reorganized by \system, sustain 0.10 and deliver
...
and switch arbitration intervene, and a workload that drives each packet $h$
hops through the hot direction divides the bound by $h$, since every hop
claims a credit at the next router. This bound is \emph{per direction}, and
...
this observation as combinational; Section~\ref{sec:timing} identifies it as
the design's optimistic timing assumption.
...
\emph{Pool ownership}. They are both called credits informally, but
Table~\ref{tab:credit-planes} shows they have different destinations and
correctness roles.
\begin{table}[t]
\centering
...
Figure~\ref{fig:credit-life} follows a single one-flit packet. The upstream router
...
sender can name all four shared slots; Table~\ref{tab:vc-map} gives the full
numbering.
\begin{table}[t]
\centering
...
preserving eight slots. We compare blind owner round-robin---which rotates
ownership without observing demand, and is distinct from the round-robin
arbitration inside the read scheduler---against pressure with caps three and
four.
...
input in the baseline; Table~\ref{tab:xbias-detail} lists the underlying
per-bias values.
...
Table~\ref{tab:cap} isolates the owner cap. Cap two prevents borrowing and
is behaviorally equivalent to static ownership. Cap three recovers most of
cap four's latency benefit while retaining one credit at the cold side. Cap
four is slightly faster here but can temporarily leave the peer with no Pool
ownership.
...
The event identities of Table~\ref{tab:events} are visible numerically:
...
The phase schedule is a real bandwidth constraint; Table~\ref{tab:negative}
collects the representative losses. Against \privatefour,
...
grant path should measure timing sensitivity. Second, the saturation knees
are located at 0.01-rate resolution with three seeds; finer 0.005 steps,
longer drain-aware runs, and five or more seeds would sharpen them further.
Third, a directed simultaneous E/W-write microbenchmark
