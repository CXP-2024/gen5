...
\begin{tikzpicture}[
  ...
  \node[box,minimum width=2.2cm] (ew) at (2.55,3.25)
    {E/W pair\\2 RES + 4-slot Pool};
  \node[box,minimum width=2.2cm] (ns) at (2.55,2.05)
    {N/S pair\\2 RES + 4-slot Pool};
  \node[box,minimum width=2.2cm] (ud) at (2.55,0.85)
    {U/D pair\\2 RES + 4-slot Pool};
  \node[ctrl,minimum width=2.4cm,minimum height=2.8cm] (sa) at (5.55,2.05)
    {Switch allocator\\phase + bank RR\\output arbitration};
  \node[box,minimum width=1.45cm,minimum height=2.8cm] (xbar) at (7.95,2.05)
  ...
  \draw[flow] (sa) -- (xbar);
  \node[anchor=east] at (1.25,3.55) {E link};
  \node[anchor=east] at (1.25,2.95) {W link};
  \node[anchor=east] at (1.25,2.25) {N/S};
  \node[anchor=east] at (1.25,1.05) {U/D};
  \draw[flow] (1.35,3.55) -- (ew.west |- 2.85,3.55);
  \draw[flow] (1.35,2.95) -- (ew.west |- 2.85,2.95);
  \draw[flow] (1.35,2.25) -- (ns.west);
  \draw[flow] (1.35,1.05) -- (ud.west);

  % Panel (b): pair datapath
  \node[font=\bfseries\small,anchor=west] at (9.25,4.25)
    {(b) One opposing-direction pair};
  \node[box,minimum width=1.25cm] (elin) at (9.75,3.20) {E input\\link};
  \node[box,minimum width=1.25cm] (wlin) at (17.80,3.20) {W input\\link};
  \node[box,minimum width=1.85cm] (eres) at (11.30,3.20)
    {E RES\\private + escape};
  \node[box,minimum width=1.85cm] (wres) at (16.25,3.20)
    {W RES\\private + escape};
  \node[ctrl,minimum width=2.35cm] (warb) at (13.78,3.20)
    {Pool write arbiter\\one grant/cycle};
  \node[poolbox,minimum width=3.95cm,minimum height=0.90cm] (pool) at (13.78,2.00)
    {Shared Pool: P0 \quad P1 \quad P2 \quad P3\\1R1W service};
  \node[ctrl,minimum width=2.55cm,minimum height=0.85cm] (state) at (10.65,0.65)
    {pair controller\\owner, busy, cap, phase, RR};
  \node[box,minimum width=2.20cm] (rmux) at (14.10,0.65)
    {phase read mux\\1 RES + 1 Pool};
  \node[box,minimum width=2.55cm] (lane) at (17.05,0.65)
    {true-ingress lane\\to switch allocator};

  \draw[flow] (elin) -- (eres);
  \draw[flow] (wlin) -- (wres);
  \draw[flow] (elin.south) -- ++(0,-0.45) -| (warb.west);
  \draw[flow] (wlin.south) -- ++(0,-0.45) -| (warb.east);
  \draw[flow] (warb) -- node[right]{1W} (pool);
  \draw[flow] (pool.south) -- node[right]{1R} (rmux.north);
  \draw[flow] (rmux) -- (lane);
  \draw[cflow] (state) -- (rmux);
  \draw[cflow] (state.north) |- (pool.west);
  \draw[cflow] (state.north) |- (eres.south);
  \draw[cflow] (state.north) -| (wres.south);
\end{tikzpicture}
}
...
remains separate; the Pool result is placed on the flit's true-ingress lane.}
...
\begin{tikzpicture}[
  ...
  credit/.style={->,very thick,dashed,draw=tidalorange!90!black}
]
  \node[font=\bfseries\small,anchor=west] at (0,2.75)
    {Allocation and forward path};
  \node[stagebox] (search) at (1.45,1.80)
    {1. Search\\private, owned, reclaim};
  \node[decision] (claim) at (4.35,1.80)
    {2. Claim slot\\owner=E, busy=1\\consume 1W token};
  \node[event] (send) at (7.30,1.80)
    {3. Send flit\\with logical Pool VC};
  \node[event] (map) at (10.25,1.80)
    {4. Map to physical slot\\store true ingress\\and upstream VC};
  \node[stagebox] (serve) at (13.25,1.80)
    {5. Phase-selected read\\SA + crossbar traversal};
  \node[event] (free) at (16.20,1.80)
  ...
  \node[font=\bfseries\small,anchor=west] at (0,0.55)
    {Feedback after release};
  \node[decision] (pressure) at (4.35,-0.25)
    {7. Compare RES pressure\\peer fuller: handoff\\tie/cap: keep};
  \node[event] (ownergrant) at (10.00,-0.25)
    {8a. Ownership grant\\new owner may allocate\\idle Pool slot};
  \node[event] (returncredit) at (15.90,-0.25)
    {8b. Occupancy credit\\returns to true sender\\on normal CreditLink};
  \draw[flow] (free.south) -- ++(0,-0.35) -| (pressure.north);
  \draw[credit] (pressure) -- node[above]{ownership plane} (ownergrant);
  \draw[credit] (free.east) -- ++(0.35,0) |-
    node[pos=.70,right]{occupancy plane} (returncredit.east);
\end{tikzpicture}
}
...
The simulator models the owner-table change with no additional grant delay.}
